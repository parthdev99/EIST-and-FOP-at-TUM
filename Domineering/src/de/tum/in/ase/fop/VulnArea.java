...
public class VulnArea {
    ...
    public static int isVulnArea(char[][] board, int x, int y, Player player) {
        ...
        int vM = player == Player.VERTICAL ? 1 : 0;
        int hM = player == Player.HORIZONTAL ? 1 : 0;
        ...
        if (board[x][y] == 'E' && board[x + hM][y + vM] == 'E') {
            ...
            if (player == Player.VERTICAL && (
                    x - 1 >= 0 && (board[x - 1][y] == 'P' || board[x - 1][y] == 'S' || board[x - 1][y] == 'D')
                            ...
                            || board[x + 1][y + 1] == 'D'))) {
                type = 2;
            } else if (player == Player.HORIZONTAL && (y > 0 && (board[x][y - 1] == 'P' || board[x][y - 1] == 'S')
        ...
        if (type > 0) {
            if (player == Player.VERTICAL && ((x <= 0 || board[x - 1][y] == 'X')
                    && (x >= board.length - 1 || board[x + 1][y] == 'X')
                    || (x <= 0 || board[x - 1][y + 1] == 'X')
                    && (x >= board.length - 1 || board[x + 1][y + 1] == 'X'))) {
                type += 2;
            }
            if (player == Player.HORIZONTAL && ((y <= 0 || board[x][y - 1] == 'X')
    ...
    public static VulnArea getVulnArea(int x, int y, Player player) {
        return new VulnArea(x, y, x + (player == Player.HORIZONTAL ? 1 : 0), y + (player == Player.VERTICAL ? 1 : 0));
    }
}
